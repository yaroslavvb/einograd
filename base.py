...
class ContractibleTensor2(Tensor):
    ...
    def __legacy_init__(index_spec_list, tensors, tag=None):
        return ContractibleTensor2(list((i, t) for (i, t) in zip(index_spec_list, tensors)))
    ...
    def __mul__(self, other: 'ContractibleTensor2') -> 'ContractibleTensor2':
        """Contraction operation"""

        assert isinstance(other, ContractibleTensor2)
        print(f'contracting {self} with {other}')
        #        self._check_indices_sorted()
        #        other._check_indices_sorted()
        ...
        right = ContractibleTensor2(list(other._original_specs), label=f'cloned_{other.label}')
        ...
        result = ContractibleTensor2(new_specs, label=f"{self.label}*{other.label}")
    ...
    def from_dense_vector(x: torch.Tensor, tag: str = None, idx: str = None):
        ...
        return ContractibleTensor2.__legacy_init__([idx + '|'], [x], tag)
    ...
    def from_dense_matrix(x: torch.Tensor, tag: str = None, idx: str = None):
        ...
        idx0 = GLOBALS.idx0
        idx1 = chr(ord(idx0)+1)
        return ContractibleTensor2.__legacy_init__([idx0+'|'+idx1], [x], tag)

    @staticmethod
    def from_dense_tensor(tensor_spec: str, x: torch.Tensor, label: str = None):
        return ContractibleTensor2([(tensor_spec, x, label)])
    ...
    def from_diag_matrix(x: torch.Tensor, tag: str = None):
        ...
        assert x.shape[0] > 0
        idx0 = GLOBALS.idx0
        return ContractibleTensor2.__legacy_init__([idx0+'|'+idx0], [x], tag)
